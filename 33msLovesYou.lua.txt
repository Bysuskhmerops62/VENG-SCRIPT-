...
local Window = Libary:MakeWindow({
    Title = "MokurenX | Brookhaven RP ",
    SubTitle = "by: Bysuskhmer",
    LoadText = "Loading MokurenX",
    Flags = "MokurenX_Broookhaven"
})
Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://17813607147", BackgroundTransparency = 0 },
...
InfoTab:AddParagraph({ "Owner / Developer:", "Lusca and vein." })
InfoTab:AddParagraph({ "Collaborations:", "Blue, sukuna, Magekko, Darkness, Star, Toddy" })
InfoTab:AddParagraph({ "You are using:", "MokurenX Brookhaven " })
...
TrollTab:AddToggle({
    Name = "Anti Sit",
    Description = "Don't Let Your Character Sit",
    Default = false,
    Callback = function(Value)
...
local function KillPlayerCouch()
    ...
    if not target or not target.Character then
        warn("Erro: Target player not found or missing character")
        return
    end
...
local function BringPlayerLLL()
    ...
    local target = Players:FindFirstChild(selectedPlayerName)
    if not target or not target.Character then
        warn("Erro: Target player not found or without characterm")
        return
    end
...
local function BringWithCouch()
    ...
    if not targetPlayer then
        warn("Erro: No target player selected")
        return
    end
    if not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        warn("Erro: Target player without character or HumanoidRootPart")
        return
    end
